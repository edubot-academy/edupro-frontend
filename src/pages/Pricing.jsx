      ...
      <section className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
          ...
          <p className="text-slate-600 mt-2 max-w-3xl mx-auto">
            Transparent pricing with flexible options. Switch/cancel anytime.
          ...
          <div className="mt-6 inline-flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-2">
            <button
              className={`px-4 py-2 rounded-xl text-sm font-semibold ${
                !yearly ? "bg-emerald-600 text-white" : "text-slate-700"
              }`}
            ...
            >
              Yearly{" "}
              {saveBadge && (
                <span className="ml-2 rounded-lg bg-emerald-100 text-emerald-700 px-2 py-0.5 text-xs">
                  {saveBadge}
                </span>
              )}
            </button>
            ...
            >
                ...
                <div className="text-3xl font-bold">
                  ...
                  <span
                    className={`text-sm ml-1 ${
                      p.featured ? "text-white/80" : "text-slate-500"
                    }`}
                  >
                    {suffix}
                  </span>
              ...
              <p
                className={`mt-2 text-sm ${
                  p.featured ? "text-white/90" : "text-slate-600"
                }`}
              >
                {p.desc}
              </p>
              ...
              <ul
                className={`mt-4 space-y-2 text-sm ${
                  p.featured ? "text-white/90" : "text-slate-700"
                }`}
              >
                {FEATURES.map((f) => (
                  <li key={f.key} className="flex items-center gap-2">
                    <span
                      className={`inline-flex h-5 w-5 items-center justify-center rounded-md text-xs font-bold ${
                        p.features[f.key]
                          ? p.featured
            ...
            {[
              {
                ...
                a: "Yes, you can upgrade or cancel anytime. Pro-rated billing applies.",
              },
              {
                q: "Is there a refund policy?",
                a: "Full refund within 7 days of cohort start if you’re not satisfied.",
              },
              {
                q: "Do you offer corporate billing?",
                a: "Yes, email billing@edupro.academy for invoices and bulk seats.",
              },
            ].map((qa, i) => (
